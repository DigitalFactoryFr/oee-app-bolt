/*
  # Improve lot matching for quality issues and stop events

  1. Changes
    - Enhance lot matching logic to better handle Excel imports
    - Add more flexible matching criteria
    - Add logging for debugging
    - Improve matching accuracy by considering time overlaps
...
BEGIN
  -- If start_time is provided, use it to find exact time match
  ...
  -- Try matching based on date and production resources
  SELECT id INTO v_lot_id
  FROM lots
  WHERE date = p_date
    AND team_member = p_team_member
  ...
  -- If still no match, try finding any lot from the same day
  SELECT id INTO v_lot_id
  FROM lots
  WHERE date = p_date
    AND (
