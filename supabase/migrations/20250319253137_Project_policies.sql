...
--------------------------------------------
-- Policy SELECT : l’utilisateur voit un projet s'il est propriétaire ou membre
...
-- Policy OWNER : seul le propriétaire peut modifier ou supprimer le projet
...
-- Policy INSERT : autoriser tout utilisateur authentifié à créer un projet
...
--------------------------------------------
-- 3. CREATION DES POLICIES POUR LA TABLE team_members
--------------------------------------------
-- Policy SELECT : l’utilisateur voit les membres des projets dont il fait partie
...
USING (
  email = auth.email()
  OR project_id IN (
    SELECT project_id FROM public.team_members WHERE email = auth.email()
  )
);
