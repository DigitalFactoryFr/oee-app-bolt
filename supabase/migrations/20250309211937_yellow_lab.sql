/*
  # Update lot matching functions for quality issues and stop events

  1. Changes
    - Improve lot matching logic to handle Excel imports
...
DECLARE
  v_lot_id uuid;
BEGIN
  -- First try exact match with start time if provided
  IF p_start_time IS NOT NULL THEN
    ...
    WHERE date = p_date
      ...
      AND (end_time IS NULL OR end_time >= p_start_time)
  ...
  -- Try matching just based on date and production resources
  SELECT id INTO v_lot_id
  FROM lots
  WHERE date = p_date
    AND team_member = p_team_member
    ...
    AND machine = p_machine
  ORDER BY 
    status = 'in_progress' DESC, -- Prefer in-progress lots
    created_at DESC -- Then take most recent
  LIMIT 1;

  -- If still no match, try matching just on date and machine
